...
func (r *PostgresRepository) GetSongs(group, song, releaseDate, text, link, createdAt, updatedAt string, limit, offset int) ([]models.Song, error) {
	...
	if group != "" {
		query += fmt.Sprintf(" AND group_name ILIKE $%d", len(args)+1)
	...
	if text != "" {
		query += fmt.Sprintf(" AND text ILIKE $%d", len(args)+1)
		args = append(args, "%"+text+"%")
	}
	if link != "" {
		query += fmt.Sprintf(" AND link ILIKE $%d", len(args)+1)
	...
	if updatedAt != "" {
		query += fmt.Sprintf(" AND DATE(updated_at) = $%d", len(args)+1)
